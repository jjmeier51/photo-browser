...
enum InstagramService {
    ...
    nonisolated static func copyToTemp(_ files: [String], handle: String, into tempFolder: URL) async -> [String] {
        ...
        // Flat, handle-prefixed names — per-handle subfolders (tried briefly) turned the
        // daily folder into a pile of folders to open one by one instead of a single
        // scrollable grid of everyone's stories.
        let cleaned = handle.replacingOccurrences(of: "/", with: "-").trimmingCharacters(in: .whitespaces)
        let prefix = cleaned.isEmpty ? "unknown" : cleaned
        var written: [String] = []
        for path in files {
            let src = URL(fileURLWithPath: path)
            let dest = tempFolder.appendingPathComponent("\(prefix)_\(src.lastPathComponent)")
