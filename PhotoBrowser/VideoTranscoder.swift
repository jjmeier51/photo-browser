...
enum VideoTranscoder {
    ...
    nonisolated static func decryptMux(videoEnc: URL, audioEnc: URL?, keyHex: String,
                                       date: Date, to dest: URL) async -> Bool {
        #if canImport(ffmpegkit)
        return await Task.detached(priority: .userInitiated) { () -> Bool in
            try? FileManager.default.removeItem(at: dest)
            var cmd = "-y -decryption_key \(keyHex) -i \"\(videoEnc.path)\""
            if let audioEnc { cmd += " -decryption_key \(keyHex) -i \"\(audioEnc.path)\"" }
            cmd += " -map 0:v:0"
            cmd += audioEnc != nil ? " -map 1:a:0" : " -map 0:a:0?"
